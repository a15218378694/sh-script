...
# 获取 HTTP 状态码（过滤 agent 日志，只保留纯数字）
# 用法: get_http_code <url>
get_http_code() {
    local url="$1"
    remote_exec "curl -s -o /dev/null -w '%{http_code}' --connect-timeout 5 ${url}" 2>/dev/null \
        | grep -oE '[0-9]+' | tail -1
}
...
HTTP_CODE=$(get_http_code "${MASTER_IP}:8443/security/license")
...
if [ "$HTTP_CODE" -ge 200 ] && [ "$HTTP_CODE" -lt 400 ] 2>/dev/null; then
    ...
    # 执行启动命令（超时10秒，防止挂起）
    remote_exec "cd /home/baidu/work/c-offline-security-server && nohup bash start/c-offline-security-server-start.sh >/dev/null 2>&1 &"
    ...
    for i in 1 2 3; do
        ...
        sleep 15
        HTTP_CODE=$(get_http_code "${MASTER_IP}:8443")
