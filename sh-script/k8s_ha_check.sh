...
'
    ...
    for item in $(get_all_deployments_all_ns); do
        ...
        # 通过 readyReplicas 检查（check_deployment_ready 内部已自动修复）
        check_deployment_ready "$ns" "$dep"
    ...
    # ----------------------------------------------------------
    # Step 4: 验证 StatefulSet 恢复状态
    # ----------------------------------------------------------
    log_step "[Step 4/4] 验证 StatefulSet 恢复状态 ..."
    echo ""
